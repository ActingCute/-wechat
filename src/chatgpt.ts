...
export class ChatGPTBot {
  ...
  async onMessage1(message: Message) {
    const talker = message.talker();
    const rawText = message.text();
    const room = message.room();
    const messageType = message.type();
    ...
    let msg =
      text == "闭嘴"
        ? "好的，我已经闭嘴，召唤我输入1"
        : await Msg.getMsg(text, message.talker()?.id || 111);
    if (msg) {
      if (privateChat) {
        return await this.trySay(talker, msg);
      } else {
        return await this.trySay(room, msg);
      }
    }
    // } else {
    //   return;
